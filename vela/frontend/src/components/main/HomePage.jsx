...
import { prettyName, isTransferOrPaydown } from './txnPretty';
...
export default function HomePage({ data, session, onAddTxn, onOpenAccount, onOpenSearch, onEditTxn, onGoTo }) {
  ...
  const recent = transactions.filter((t) => t.date >= sevenDaysIso).slice(0, 5);
  const hiddenRecent = Math.max(0, transactions.filter((t) => t.date >= sevenDaysIso).length - recent.length);
        ...
        {recent.length > 0 && (
          <button
            ...
            style={{
              ...
              color: 'var(--t2)',
              borderTop: '1px solid var(--b1)', marginTop: 6,
            }}
          >
            {hiddenRecent > 0 ? `View all transactions (${hiddenRecent}+ more)` : 'View all transactions'}
          </button>
        )}
      </div>
