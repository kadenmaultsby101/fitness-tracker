...
Trace is a separate app that lives in this `trace/` folder. The Pulse fitness
tracker at the repo root is unaffected.
...
`file://`). Pick any static server, run it from inside `trace/`:
...
The project is already static — no build step needed. Deploy the `trace/`
folder as its own Vercel project (separate from Pulse at the repo root):
...
You can also drag-and-drop just the `trace/` folder into the Vercel dashboard
...
Because the key is scoped to Trace's deployed origin, it can't collide with
the Pulse fitness tracker even if both are hosted on related domains.
